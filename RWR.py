...
def parse_arguments():
    ...
    parser.add_argument(
        "--nodes", type=Path, required=True, help="Path to the nodes file"
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def RWR(network_file: Path, nodes_file: Path, alpha: float, max_iter: int, output_file: Path):
    ...
    scores = nx.pagerank(
        graph,
        personalization={n: 1 for n in nodelist},
        alpha=alpha,
        max_iter=max_iter,
        weight="weight",
    )
...
def main():
    ...
    RWR(args.network, args.nodes, args.alpha, args.max_iter, args.output)
